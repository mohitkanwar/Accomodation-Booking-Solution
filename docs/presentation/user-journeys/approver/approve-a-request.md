...
**Outcome:** Authorise a compliant request to proceed to booking operations.
...
- Confirm that the request is complete and within policy, or that an exception is justified.
- Add an optional decision note.
- Approve the request.
- Notify the traveller and route the request to the booking-operator queue.
- Retain the policy snapshot, decision, actor, and timestamp in the audit history.
...
```plantuml-image
../../diagrams/user-journeys/approver/approve-a-request/01-end-to-end-interaction.puml | Approver — Approve a request — End-to-end interaction
```
