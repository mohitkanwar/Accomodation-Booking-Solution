...
sidebar_label: "Reassess a materially changed offer"
description: "Make a fresh decision when price, availability, or booking conditions change before fulfilment."
---

# Reassess a materially changed offer

**Primary actor:** Approver<br />
**Outcome:** Make a fresh decision when price, availability, or booking conditions change before fulfilment.
...
- Review the operator's explanation and the replacement offer.
- Compare the changed price and conditions with the original approval and policy snapshot.
- Approve the revised offer, reject it, or seek clarification.
- Preserve both decisions and their related offer versions.
...
```plantuml-image
../../diagrams/user-journeys/approver/reassess-a-materially-changed-offer/01-end-to-end-interaction.puml | Approver — Reassess a materially changed offer — End-to-end interaction
```
...
|---|---|---|---|---|
| `GET` | `/api/v1/approvals/{approvalId}/changed-offer` | Return original and replacement offer versions with material differences. | Booking Workflow Service | Not required |
| `POST` | `/api/v1/approvals/{approvalId}/decisions` | Record approve or reject as an immutable, versioned decision. | Booking Workflow Service | Required: decisionId and expectedVersion |
...
{
  "requestId": "01JEXAMPLE...",
  "id": "reassess-a-materially-changed-offer-123",
  "status": "accepted",
  "version": 8,
  "occurredAt": "2026-07-25T12:00:00Z",
  "links": {
    "self": "/api/v1/approvals/resource-123/decisions",
    "allowedActions": [
      "view"
    ]
  }
