...
sidebar_label: "Reject a request"
...
**Outcome:** Stop a request that is unsupported, incomplete, or outside acceptable policy.
...
- Select a rejection category and provide a clear reason.
- Reject the request and prevent downstream booking.
- Notify the traveller with the decision and available next action.
- Retain the rejected request and decision evidence for reporting and audit.
...
```plantuml-image
../../diagrams/user-journeys/approver/reject-a-request/01-end-to-end-interaction.puml | Approver — Reject a request — End-to-end interaction
```
