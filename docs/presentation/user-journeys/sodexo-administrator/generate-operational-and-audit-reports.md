...
sidebar_label: "Generate operational and audit reports"
...
**Outcome:** Provide authorised insight into adoption, service performance, compliance, and privileged activity.
...
- Select the reporting period, corporate scope, region, and data classification.
- Generate adoption, approval, booking, failure, reconciliation, and audit reports.
- Apply masking and access controls to personal and commercially sensitive data.
- Export or schedule the report for authorised recipients.
- Record report generation and distribution where required.
...
```plantuml-image
../../diagrams/user-journeys/sodexo-administrator/generate-operational-and-audit-reports/01-end-to-end-interaction.puml | Sodexo Administrator — Generate operational and audit reports — End-to-end interaction
```
...
|---|---|---|---|---|
| `POST` | `/api/v1/platform/reports` | Generate scoped operational, failure, reconciliation, or audit data. | Accommodation Reporting Service | Not required |
| `POST` | `/api/v1/platform/reports/{reportId}/deliveries` | Export or schedule a masked report for authorised recipients. | Accommodation Reporting Service | Required: deliveryId |
...
`POST /api/v1/platform/reports/{reportId}/deliveries`.
...
```json
{
  ...
  "id": "generate-operational-and-audit-reports-123",
  "status": "accepted",
  "version": 8,
  "occurredAt": "2026-07-25T12:00:00Z",
  "links": {
    "self": "/api/v1/platform/reports/resource-123/deliveries",
    "allowedActions": [
      "view"
    ]
  }
