...
sidebar_label: "Onboard and maintain a corporate tenant"
description: "Create the client boundary and connect its core identity and operational configuration."
---

# Onboard and maintain a corporate tenant

**Primary actor:** Sodexo Administrator<br />
**Outcome:** Create the client boundary and connect its core identity and operational configuration.
...
- Create the corporate tenant and record its legal, regional, and support details.
- Configure identity mapping, organisation identifiers, currencies, and localisation.
- Assign the initial corporate administrators and operator scope.
- Validate the setup in a controlled environment.
- Activate the tenant and retain an auditable onboarding record.
...
```plantuml-image
../../diagrams/user-journeys/sodexo-administrator/onboard-and-maintain-a-corporate-tenant/01-create-and-configure-tenant.puml | Sodexo Administrator — Onboard and maintain a corporate tenant — Create and configure tenant
```

### 2. Activate validated tenant

```plantuml-image
../../diagrams/user-journeys/sodexo-administrator/onboard-and-maintain-a-corporate-tenant/02-activate-validated-tenant.puml | Sodexo Administrator — Onboard and maintain a corporate tenant — Activate validated tenant
```
...
|---|---|---|---|---|
| `POST` | `/api/v1/platform/tenants` | Create an inactive corporate tenant and regional configuration. | Tenant Administration Service | Required: tenantExternalId |
| `POST` | `/api/v1/platform/tenants/{tenantId}/validation` | Validate identity mappings, localisation, users, and configuration. | Tenant Administration Service | Not required |
| `POST` | `/api/v1/platform/tenants/{tenantId}/activate` | Activate a fully validated tenant. | Tenant Administration Service | Required: expectedVersion |
...
```json
{
  ...
  "identityMapping": {
    "issuer": "existing-b2c-idp",
    "organisationClaim": "client_id"
  }
}
```
...
{
  "requestId": "01JEXAMPLE...",
  "id": "onboard-and-maintain-a-corporate-tenant-123",
  "status": "accepted",
  "version": 8,
  "occurredAt": "2026-07-25T12:00:00Z",
  "links": {
    "self": "/api/v1/platform/tenants/resource-123/activate",
    "allowedActions": [
      "view"
    ]
  }
