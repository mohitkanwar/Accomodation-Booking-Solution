...
sidebar_label: "Manage travel policies"
...
**Outcome:** Create, review, publish, and retire versioned corporate travel rules.
...
- Review the currently effective policy and its activation dates.
- Create a draft policy version without changing historical requests.
- Configure eligibility, advance-booking, stay-duration, exception, and approval rules.
- Preview the impact on representative traveller and destination scenarios.
- Publish the policy with an effective date and retain the previous version for audit.

## End-to-end sequence

### 1. Draft and simulate policy

```plantuml-image
../../diagrams/user-journeys/corporate-administrator/manage-travel-policies/01-draft-and-simulate-policy.puml | Corporate Administrator — Manage travel policies — Draft and simulate policy
```

### 2. Publish effective version

```plantuml-image
../../diagrams/user-journeys/corporate-administrator/manage-travel-policies/02-publish-effective-version.puml | Corporate Administrator — Manage travel policies — Publish effective version
```
...
|---|---|---|---|---|
| `GET` | `/api/v1/admin/policies` | List effective, draft, scheduled, and retired corporate policy versions. | Accommodation Service | Not required |
| `POST` | `/api/v1/admin/policies` | Create a new immutable draft derived from an effective version. | Accommodation Service | Required: Idempotency-Key |
| `POST` | `/api/v1/admin/policies/{policyId}/preview` | Evaluate representative scenarios without publishing the draft. | Accommodation Service | Not required |
| `POST` | `/api/v1/admin/policies/{policyId}/publish` | Schedule a validated policy version for activation. | Accommodation Service | Required: expectedVersion |
...
{
  "requestId": "01JEXAMPLE...",
  "id": "manage-travel-policies-123",
  "status": "accepted",
  "version": 8,
  "occurredAt": "2026-07-25T12:00:00Z",
  "links": {
    "self": "/api/v1/admin/policies/resource-123/publish",
    "allowedActions": [
      "view"
    ]
  }
