...
sidebar_label: "Manage price ranges and exceptions"
description: "Set destination-specific nightly limits for each corporate role level."
---

# Manage price ranges and exceptions

**Primary actor:** Corporate Administrator<br />
**Outcome:** Set destination-specific nightly limits for each corporate role level.
...
- Select a role level, destination, currency, and effective period.
- Define the permitted nightly range and any tax or fee treatment.
- Configure warning, hard-stop, and exception-approval behaviour.
- Compare the new range with current market and booking data.
- Publish the versioned price rule and retain its change history.

## End-to-end sequence

### 1. Create or revise price rule

```plantuml-image
../../diagrams/user-journeys/corporate-administrator/manage-price-ranges-and-exceptions/01-create-or-revise-price-rule.puml | Corporate Administrator — Manage price ranges and exceptions — Create or revise price rule
```

### 2. Publish effective rule

```plantuml-image
../../diagrams/user-journeys/corporate-administrator/manage-price-ranges-and-exceptions/02-publish-effective-rule.puml | Corporate Administrator — Manage price ranges and exceptions — Publish effective rule
```
...
|---|---|---|---|---|
| `GET` | `/api/v1/admin/price-rules` | List price limits by role, destination, currency, and effective period. | Accommodation Service | Not required |
| `PUT` | `/api/v1/admin/price-rules/{ruleId}` | Save a versioned nightly range and exception behaviour. | Accommodation Service | Required: expectedVersion |
| `POST` | `/api/v1/admin/price-rules/{ruleId}/publish` | Publish a non-overlapping effective price-rule version. | Accommodation Service | Required: commandId |
...
`POST /api/v1/admin/price-rules/{ruleId}/publish`.
...
{
  "requestId": "01JEXAMPLE...",
  "id": "manage-price-ranges-and-exceptions-123",
  "status": "accepted",
  "version": 8,
  "occurredAt": "2026-07-25T12:00:00Z",
  "links": {
    "self": "/api/v1/admin/price-rules/resource-123/publish",
    "allowedActions": [
      "view"
    ]
  }
