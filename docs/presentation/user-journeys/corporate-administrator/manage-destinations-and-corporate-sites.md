...
sidebar_label: "Manage destinations and corporate sites"
description: "Control which office destinations and sites are available to the corporation."
---

# Manage destinations and corporate sites

**Primary actor:** Corporate Administrator<br />
**Outcome:** Control which office destinations and sites are available to the corporation.
...
- Review countries, cities, offices, and permitted accommodation destinations.
- Add a destination and associate it with one or more corporate sites.
- Configure availability dates, booking guidance, and destination-specific restrictions.
- Temporarily suspend or retire a destination.
- Preview the destination list visible to eligible travellers.

## End-to-end sequence

### 1. Maintain destination configuration

```plantuml-image
../../diagrams/user-journeys/corporate-administrator/manage-destinations-and-corporate-sites/01-maintain-destination-configuration.puml | Corporate Administrator — Manage destinations and corporate sites — Maintain destination configuration
```

### 2. Publish traveller-visible destination

```plantuml-image
../../diagrams/user-journeys/corporate-administrator/manage-destinations-and-corporate-sites/02-publish-traveller-visible-destination.puml | Corporate Administrator — Manage destinations and corporate sites — Publish traveller-visible destination
```
...
|---|---|---|---|---|
| `GET` | `/api/v1/admin/destinations` | List destination and site configuration with effective status. | Accommodation Service | Not required |
| `PUT` | `/api/v1/admin/destinations/{destinationId}` | Create or update a corporate destination and site mapping. | Accommodation Service | Required: expectedVersion |
| `POST` | `/api/v1/admin/destinations/{destinationId}/publish` | Activate, suspend, or retire a destination version. | Accommodation Service | Required: commandId |
...
{
  "requestId": "01JEXAMPLE...",
  "id": "manage-destinations-and-corporate-sites-123",
  "status": "accepted",
  "version": 8,
  "occurredAt": "2026-07-25T12:00:00Z",
  "links": {
    "self": "/api/v1/admin/destinations/resource-123/publish",
    "allowedActions": [
      "view"
    ]
  }
