...
sidebar_label: "Provide booking support"
...
**Outcome:** Investigate reservation, property, or guest-service issues raised through the support channel.
...
- Review the booking reference, issue category, and supporting evidence.
- Validate the reservation state in the provider's system.
- Coordinate with the property or distribution-channel support team.
- Provide a resolution, workaround, or escalation with an expected timeline.
- Record the final outcome and communicate it through the connected support process.
...
```plantuml-image
../../diagrams/user-journeys/provider-agent/provide-booking-support/01-end-to-end-interaction.puml | Accommodation Provider Agent — Provide booking support — End-to-end interaction
```
...
| `POST` | `/provider-api/v1/reservations/{reservationId}/support-updates` | Return evidence, resolution, or escalation through the support channel. | Provider Support Service<br/>Booking.com / Sodexo support channel | Required: updateId |
...
`POST /provider-api/v1/reservations/{reservationId}/support-updates`.
...
{
  "requestId": "01JEXAMPLE...",
  "id": "provide-booking-support-123",
  "status": "accepted",
  "version": 8,
  ...
  "links": {
    "self": "/provider-api/v1/reservations/resource-123/support-updates",
    "allowedActions": [
      "view"
    ]
  }
