...
sidebar_label: "Process an amendment or cancellation"
description: "Apply supported booking changes and communicate their financial impact."
---

# Process an amendment or cancellation

**Primary actor:** Accommodation Provider Agent<br />
**Outcome:** Apply supported booking changes and communicate their financial impact.
...
- Locate the reservation using the supplier or channel reference.
- Review the requested change against availability and booking conditions.
- Calculate any price difference, fee, refund, or restriction.
- Confirm or reject the change through the provider channel.
- Retain the revised reservation state and commercial evidence.
...
```plantuml-image
../../diagrams/user-journeys/provider-agent/process-an-amendment-or-cancellation/01-end-to-end-interaction.puml | Accommodation Provider Agent — Process an amendment or cancellation — End-to-end interaction
```
...
| `POST` | `/provider-api/v1/reservations/{reservationId}/changes` | Apply or reject an amendment/cancellation and return commercial impact. | Provider PMS / Channel Manager<br/>Booking.com distribution channel | Required: changeId |
...
`POST /provider-api/v1/reservations/{reservationId}/changes`.
...
{
  "requestId": "01JEXAMPLE...",
  "id": "process-an-amendment-or-cancellation-123",
  "status": "accepted",
  "version": 8,
  ...
  "links": {
    "self": "/provider-api/v1/reservations/resource-123/changes",
    "allowedActions": [
      "view"
    ]
  }
