...
sidebar_label: "Manage accommodation configuration"
description: "Maintain the property content and booking conditions exposed through the provider channel."
---

# Manage accommodation configuration

**Primary actor:** Accommodation Provider Agent<br />
**Outcome:** Maintain the property content and booking conditions exposed through the provider channel.
...
- Maintain property identity, address, facilities, room types, and guest policies.
- Configure rates, taxes, fees, cancellation terms, and payment conditions.
- Update availability and temporary sales restrictions.
- Validate how the accommodation appears through the distribution channel.
- Publish the configuration with its effective timing and change history.
...
```plantuml-image
../../diagrams/user-journeys/provider-agent/manage-accommodation-configuration/01-end-to-end-interaction.puml | Accommodation Provider Agent — Manage accommodation configuration — End-to-end interaction
```
...
|---|---|---|---|---|
| `PUT` | `/provider-api/v1/properties/{propertyId}` | Publish property, room, rate, policy, and availability configuration. | Provider PMS / Channel Manager<br/>Booking.com provider distribution channel | Required: configurationVersion |
...
`PUT /provider-api/v1/properties/{propertyId}`.
...
{
  "requestId": "01JEXAMPLE...",
  "id": "manage-accommodation-configuration-123",
  "status": "accepted",
  "version": 8,
  ...
  "links": {
    "self": "/provider-api/v1/properties/resource-123",
    "allowedActions": [
      "view"
    ]
  }
